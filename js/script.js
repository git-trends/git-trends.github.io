...
function render404Page() {
  ...
  app.innerHTML = `<div class="not-found">
    ...
    <p class="not-found-desc">The page <span style="font-family:var(--font-mono);color:var(--fg)">${escapeHtml(route)}</span> does not exist. It may have been moved or removed, or you may have mistyped the URL.</p>
...
function updateNav(route) {
  document.querySelectorAll("[data-nav]").forEach((link) => {
    ...
    if (navId === "trending" && (route === "/" || route === ""))
      isActive = true;
    else if (navId === "search" && route.startsWith("/search")) isActive = true;
    else if (navId === "about" && route.startsWith("/about")) isActive = true;
